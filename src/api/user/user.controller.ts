...
import { contactUsEmail, verifyEmailToResetPassword } from '../../utils/email'
...
export async function createUserHandler (req: Request, res: Response) {
  const { email, password, isVerified }: { email: string, password: string, isVerified: boolean } = req.body
  try {
    const existingUser: DocumentDefinition<UserDocument> | null = await getUserByEmail(email)
